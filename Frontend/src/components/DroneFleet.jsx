...
import { Zap, MapPin, Battery, Signal, AlertTriangle, Play, Pause, RotateCcw, Map, Users, Wifi, ArrowLeft } from 'lucide-react';
...
export default function DroneFleet() {
        ...
        setDrones(prev => {
          ...
          Object.keys(data || {}).forEach(droneId => {
            if (updated[droneId]) {
              updated[droneId] = {
                ...updated[droneId],
                detection: data[droneId].detection ?? updated[droneId].detection,
                confidence: typeof data[droneId].confidence === 'number' ? data[droneId].confidence : updated[droneId].confidence
              };
            }
          });
          return updated;
  ...
  const getBatteryColor = (battery) => {
    if (typeof battery !== 'number') return 'bg-slate-500';
    if (battery > 50) return 'bg-emerald-400';
    if (battery > 20) return 'bg-yellow-400';
    return 'bg-red-400';
  };

  // Optimistic UI: manual RTH updates local state immediately and still calls backend
  const handleManualRTH = async (droneId) => {
    setDrones(prev => {
      if (!prev || !prev[droneId]) return prev;
      return {
        ...prev,
        [droneId]: {
          ...prev[droneId],
          returning_home: true,
          rth_reason: 'manual_command',
    ...
    try {
      await fetch(`/api/drones/${encodeURIComponent(droneId)}/rth`, { method: 'POST' });
      alert(`RTH initiated for ${droneId}`);
    } catch (error) {
      console.error('RTH failed (backend):', error);
      alert(`RTH request failed for ${droneId} (backend). UI shows local returning state.`);
    }
  };

  // Optimistic UI: emergency RTH for all active drones
  const handleEmergencyRTHAll = async () => {
    if (!confirm('Emergency RTH for ALL drones?')) return;

    setDrones(prev => {
      const updated = { ...prev };
      Object.keys(updated).forEach(id => {
        if (!updated[id]) return;
        // mark active/inactive as returning (you can adjust condition)
        updated[id] = {
          ...updated[id],
          returning_home: true,
          rth_reason: 'emergency_all',
          emergency_mode: true,
          status: 'returning'
        };
      });
    ...
    try {
      await fetch('/api/drones/emergency-rth-all', { method: 'POST' });
      alert('Emergency RTH initiated for all drones!');
    } catch (error) {
      console.error('Emergency RTH failed (backend):', error);
      alert('Emergency RTH command failed on backend. UI still shows local emergency state.');
    }
  };
  ...
  const handleMissionSubmit = (e) => {
    ...
    setDrones(prev => {
      if (!prev || !prev[missionForm.droneId]) return prev;
      return {
        ...prev,
        [missionForm.droneId]: {
  ...
  // small helpers
  const getRTHStatus = (drone) => {
    if (drone && drone.returning_home) {
      return (
        <div className="flex items-center gap-1 text-orange-400">
          <ArrowLeft className="w-3 h-3" />
          <span className="text-xs">RTH - {drone.rth_reason ?? 'reason unknown'}</span>
        </div>
      );
    }
    return null;
  ...
  const getBatteryStatus = (battery) => {
    if (typeof battery !== 'number') return { color: 'bg-slate-500', status: 'UNKNOWN' };
    if (battery <= 10) return { color: 'bg-red-500', status: 'CRITICAL' };
    if (battery <= 20) return { color: 'bg-orange-500', status: 'LOW' };
    if (battery <= 50) return { color: 'bg-yellow-500', status: 'MEDIUM' };
    return { color: 'bg-green-500', status: 'GOOD' };
  };

  // Derived arrays for rendering and map
  const dronesArray = Object.values(drones);
  const activeCount = dronesArray.filter(d => d.status === 'active').length;
  const avgBattery = Math.round(dronesArray.reduce((acc, d) => acc + (typeof d.battery === 'number' ? d.battery : 0), 0) / dronesArray.length || 0);
    ...
    <div className="min-h-screen bg-gray-900 text-slate-100 antialiased">
      ...
      <main className="p-6 grid grid-cols-12 gap-6">
        ...
        <section className="col-span-3 bg-gradient-to-b from-slate-800/40 to-slate-900/30 rounded-2xl p-4 border border-slate-800/60 shadow-lg">
          <h3 className="text-sm font-semibold text-slate-300 mb-4">Drone Fleet ({dronesArray.length})</h3>
          ...
          <div className="space-y-2 max-h-[42vh] overflow-auto pr-2">
            {dronesArray.map((d) => (
              <motion.div
                ...
                className={`flex flex-col bg-slate-800/30 p-3 rounded-lg border cursor-pointer transition-all ${selected === d.drone_id ? 'border-emerald-500/50 bg-emerald-900/20' : 'border-slate-700/40'}`}
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div>
                    ...
                    <div className="text-xs text-slate-400">
                      BATT {d.battery ?? 'N/A'}% · {d.alt ? Math.round(d.alt) : 'N/A'}m
                    </div>
                  </div>
                ...
                <div className="w-full bg-slate-700 rounded-full h-1.5 mb-2">
                  <div 
                    ...
                    style={{ width: `${Math.max(typeof d.battery === 'number' ? d.battery : 5, 5)}%` }}
                  ></div>
                </div>
                ...
                <div className="flex gap-2 mt-2">
                  {d.status === 'active' && !d.returning_home && (
                    <button
                      onClick={(e) => { e.stopPropagation(); handleManualRTH(d.drone_id); }}
                  ...
                  {(typeof d.battery === 'number' && d.battery <= 20) && (
        ...
        <section className="col-span-6 rounded-2xl overflow-hidden border border-slate-800/50 shadow-lg bg-gradient-to-br from-slate-900/10 to-slate-900/5">
          <div className="h-[62vh] relative">
            <DroneMap drones={dronesArray} selectedDrone={selected} />
        ...
        <section className="col-span-3 rounded-2xl p-4 border border-slate-800/40 bg-slate-900/10 shadow-inner">
          ...
          <div className="space-y-2 text-xs max-h-[50vh] overflow-auto">
            {dronesArray.map((d) => (
              <div key={d.drone_id} className={`p-3 rounded-lg border ${d.detection === 'Human' ? 'bg-red-900/20 border-red-500/30' : 'bg-slate-800/20 border-slate-700/30'}`}>
                ...
                <div className="flex justify-between items-center mt-1">
                  ...
                  <span className="text-slate-300">{typeof d.lat === 'number' ? d.lat.toFixed(3) : 'N/A'}, {typeof d.lon === 'number' ? d.lon.toFixed(3) : 'N/A'}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 space-y-2">
            ...
            <button className="w-full bg-red-600 hover:bg-red-700 text-white py-2 px-3 rounded-lg text-sm font-medium transition-colors" onClick={handleEmergencyRTHAll}>
              Emergency RTH
            </button>
          </div>
        </section>
